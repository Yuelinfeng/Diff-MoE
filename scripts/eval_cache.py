...
BUILD_DIR = os.environ.get("DIFF_MOE_BUILD_DIR", os.path.join(PROJECT_ROOT, "build"))
DATA_DIR = os.environ.get("DIFF_MOE_DATA_DIR", "/data")
FT_DATA_DIR = os.path.join(DATA_DIR, "ft")
LOG_ROOT = os.environ.get("DIFF_MOE_LOG_DIR", os.path.join(PROJECT_ROOT, "logs"))
CPP_CONFIG_PATH = os.environ.get("DIFF_MOE_CPP_CONFIG", os.path.join(PROJECT_ROOT, "cpp_config.ini"))
PERF_BENCHMARK_PATH = os.path.join(PROJECT_ROOT, "examples", "pytorch", "t5", "perf_benchmark.py")
...
def get_model_config(model, cache_size):
    ...
    if model in ["switch-base-128", "switch-large-128"]:
        ...
        adapter_path = os.path.join(DATA_DIR, model)
...
def profile_config(cpp_config, model, method, batch_size, cache_ratio=0, cache_policy="LFU", dataset_name="", use_moecache=True, cache_size=6, fix_cache_size=0, max_val=2, threshold=1.0, dec_in_cache=0.4, dec_out_cache=0.4, print_log=False):
    iterations = 4
    os.makedirs(os.path.join(LOG_ROOT, model), exist_ok=True)
    ...
    cpp_config["default"] = {
        ...
        "profiling": "1",
        "detailed_timing": "0",
        "offload_path": os.path.join(FT_DATA_DIR, model) + "/",
        "load_from_cpp": "1",
        "quant_mode": "0",
        "vocab_size": "32128",
    ...
    command = (
        ...
        f"--ckpt_path {os.path.join(FT_DATA_DIR, model)}/ "
        f"--model {base_model} "
        f"--duration 0 "
    ...
    with open(os.path.join(LOG_ROOT, f"{exp_name}.log"), "w") as fp:
...
def main():
    ...
    cpp_config.read(CPP_CONFIG_PATH)
