...
RUN_NAME="${RUN_NAME:-sampling_scaleup_$(date +%Y%m%d_%H%M%S)}"
...
Run name: $RUN_NAME
Goal: scale up the current best sampling config on 4090D
Checkpoint: $CKPT_DIR
Library: $LIB_PATH
...
run_one "S1" "batch4" "4" "1" "32"
run_one "S2" "seq96" "1" "1" "96"

echo "Completed sampling scale-up validation. Summary written to $SUMMARY_FILE"
